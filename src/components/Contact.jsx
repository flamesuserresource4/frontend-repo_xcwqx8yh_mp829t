  ...
  return (
    <section id="contact" className="py-20 bg-slate-50">
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-10">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-rose-950">Контакты</h2>
          ...
          <motion.div
            ...
            className="mt-6 rounded-2xl overflow-hidden ring-1 ring-slate-200 bg-white"
          >
        ...
        >
          <form onSubmit={submit} className="space-y-4">
            <div>
              ...
              <input required value={form.name} onChange={e=>setForm({...form, name:e.target.value})} className="w-full mt-1 px-3 py-2 rounded-xl ring-1 ring-slate-300 focus:ring-rose-400 outline-none" />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-slate-600">Телефон</label>
                <input value={form.phone} onChange={e=>setForm({...form, phone:e.target.value})} className="w-full mt-1 px-3 py-2 rounded-xl ring-1 ring-slate-300 focus:ring-rose-400 outline-none" />
              </div>
              <div>
                <label className="text-sm text-slate-600">Email</label>
                <input type="email" value={form.email} onChange={e=>setForm({...form, email:e.target.value})} className="w-full mt-1 px-3 py-2 rounded-xl ring-1 ring-slate-300 focus:ring-rose-400 outline-none" />
              </div>
            </div>
            <div>
              <label className="text-sm text-slate-600">Сообщение</label>
              <textarea required rows="4" value={form.message} onChange={e=>setForm({...form, message:e.target.value})} className="w-full mt-1 px-3 py-2 rounded-xl ring-1 ring-slate-300 focus:ring-rose-400 outline-none" />
            </div>
            <button className="w-full px-4 py-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-medium">Отправить</button>
